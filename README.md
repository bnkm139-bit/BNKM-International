...
1. Create a GitHub repo and push these files to the `main` branch:
       ...
       git branch -M main
       git remote add origin https://github.com/bnkm139-bit/<repo>.git
...
1. `CNAME` (repo root) -- currently `bnkminternational.co.uk`. Put your real
   domain here (apex like `bnkminternational.co.uk`, or a subdomain like
   `www.bnkminternational.co.uk`). GitHub Pages reads this file on every deploy.
...
Apex domain (bnkminternational.co.uk):
...
www subdomain (www.bnkminternational.co.uk):

    CNAME  www   bnkm139-bit.github.io.
